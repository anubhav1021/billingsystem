...
public class paybill extends JFrame implements ActionListener {
    ...
    paybill(String meter){
        ...
        try{
            ...
            ResultSet resultSet = c.statement.executeQuery("select * from new_customer1 where meter_no = '"+meter+"'");
        ...
        searchmonthcho.addItemListener(new ItemListener() {
            ...
            public void itemStateChanged(ItemEvent e) {
                databaseconn c = new databaseconn();
                try{
                    ResultSet resultSet = c.statement.executeQuery("select * from bill1 where meter_no = '"+meter+"' and month = '"+searchmonthcho.getSelectedItem()+"'");
                    while (resultSet.next()){
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource()==pay){
            try{
                databaseconn c = new databaseconn();
                c.statement.executeUpdate("update bill1 set status = 'Paid' where meter_no = '"+meter+"' and month = '"+searchmonthcho.getSelectedItem()+"'");
            }catch (Exception E){
                E.printStackTrace();
            }
            setVisible(false);
